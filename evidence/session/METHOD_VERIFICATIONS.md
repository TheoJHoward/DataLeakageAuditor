...
> **RETIRED AT R229 §0 — `15dc83c78950d42b…` is no longer quoted as evidence.**
> Sixteen conventions over the five stored bodies, none producing it; the bodies
> agree with one another, so every finding below stands, but the number cannot be
> regenerated by anyone. It is replaced by
> `ddb133ff2fc959f0efb24124ca4d0f9dfc70f528cd5e1892136436c8dc34d9f1`, measured at
> the declared floor corner (CPython 3.11.9, numpy 1.26.4, pandas 2.1.4, pyarrow
> 14.0.2) and at the current resolution (CPython 3.12.10, numpy 2.4.2, pandas
> 3.0.1, pyarrow 23.0.1) by `tools/portability_digest.py`, which prints its own
> recipe. The tables below are left as they were written: they record what was
> measured on their date, and rewriting a dated measurement to carry a later
> number would be worse than retiring it in place. **`INSTALL.md` carries the
> retirement in full.**
...
| the pipeline digest — now `ddb133ff…` | **established stable, strongly**, and its predecessor is RETIRED | `15dc83c7…` was quoted across five environments and cannot be regenerated by anyone (R229 §0); `ddb133ff…` replaces it, measured at the declared floor corner and at the current resolution by `tools/portability_digest.py`, which emits its own recipe |
