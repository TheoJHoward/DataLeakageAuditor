...
| Any commit message not passed with `-F` **from a file**; `git commit -F -`, or `-F` with anything but a path to a file on disk. | R160; tightened R220 | `tools/safe_edit.commit`, and since R268 a **commit-msg hook** (`.githooks/commit-msg`, `core.hooksPath` checked by `certify_preconditions`) that refuses a message lacking the trailer `safe_edit.commit` appends. **A tripwire, not a proof:** git does not tell a hook whether `-m` or `-F` was used, so a trailer typed by hand passes. It closes the accidental route, which is the one that broke at R267. **Merges: none, and no merge path is built.** Single-author linear history is the practice (R269 §0(b)); a merge would be refused by the hook, and a route for one is built when one is first needed, not before. The hook needs `py` or `python3` on the clone, which INSTALL.md says. |
| A programmatic edit that changes a file's line endings. | R220 | `tools/safe_edit.edit` |
| Backticks in any shell argument. | R143 | **In COMMIT MESSAGES, closed by the R268 hook**, because a message that came from a file never passed through a shell. **In every OTHER shell argument, still operator discipline and no mechanism** — no hook sees a shell, and the commit-msg hook is not to be read as covering more than commit messages. |
...
| **THE CLOSING SEQUENCE IS: commit, then the step set in `tools/certify_preconditions.py` IN ITS ORDER on the committed tree, then push, then REMOTE AT HEAD, then report.** *"Remote at head"* is its own step and its own fact: `git rev-parse origin/main` equals `HEAD`, READ, never inferred from a push's exit status. The close table carries it as a row, and **a round that ends with the remote behind says so in the report's first line.** | R262, corrected R263 | R262– |
| **Why this cites the step set instead of listing it, and what listing it cost.** R262 wrote the sequence as *suite, certification, commit, push, remote at head, report* — **with the suite BEFORE the commit**, while `certify_preconditions.py`'s step set has `clean_tree.py` first and the suite fifth, i.e. after the tree is attested. Two lists for one sequence, and they disagreed about the one thing that matters: which tree the suite runs on. **At R261 the suite ran on the working tree with that round's new module untracked, and certification then attested the committed tree** — both correct at the moment each ran, and nothing checked they were the same tree. The module was outside the suite's own population (D-V30A-102). `clean_tree.py` was never at fault: measured at R263, it counts an untracked non-ignored file as dirty and exits 1. **The rule now names the mechanism and stops carrying a second copy of its order**, which is this file's own stated design. | R263 | R263– |
| **Ending a turn with that sequence still owed, where continuation was available.** Do not end on a notification; do not wait for the next delta. **The tie goes to continuation, and it goes that way on both sides.** Theo is never the one who has to say "nothing is running." | R258, corrected R262 | R258– |
...
| **Why the sequence gained a step at R262, and what leaving it out cost.** The row above listed the closing sequence as *suite, certification, commit, report* and named no push. **The remote sat at R236's commit for twenty-nine rounds while the work stayed on one disk**, and no report said "not pushed" because no step asked. Measured when it was finally noticed: one push moved `a93b82d..49dda6b` — thirty-one commits. **A rule that omits a step licenses omitting it**, and the omission was invisible from inside the sequence: every round's report was complete against the list it was checked against. The other half is a reading, not a rule — a push's exit status is not the remote's position, and the range git prints has a left-hand side that says where the remote actually was. Asking what that left-hand side meant is what found this. | R262 | R262– |
| **A correctness suite on toy data cannot see a complexity regression.** The instruments that run at fixture scale are this project's only complexity instruments, and **they report by not returning** rather than by failing. **The mechanism, because vigilance is not one:** every instrument that runs at scale prints its own wall time beside its figures (R228 — a figure carries its frame), and a run exceeding the last recorded time by an order of magnitude is **stopped and looked at, not waited out.** | R262 | R262– |
| **The instance that produced it, and why nothing else could have caught it.** R261's first batch accumulator looped over corrupted cells in Python with a `pd.Timestamp` built per cell. It was correct, and it runs once per column of every declared frame — the acceptance fixture's two frames carry 464,199 × 5 and 397,457 × 11 cells. **The whole-frame guard was still running eighteen hours later on 365 seconds of CPU**, and was stopped rather than finished. Every unit test passed in under a second, on twelve-row frames; **no test in the suite could have seen it, because none of them is large enough to.** Grouped instead of iterated, the same work is 1.36 s at fixture scale. The eighteen hours were the cost of having no rule here. | R262 | R262– |
| **PROBE-PATH EDITS ARE BATCHED AHEAD OF ONE GUARD RUN PER ROUND.** Every edit to a file in `evidence/session/PROBE_PATH_SET.json`'s `path_set` voids the last whole-frame guard result, because **a guard result is a claim about ONE tree state**. So a round's probe-path work is gathered first and the guard runs once over all of it. **Work that touches no file in the path set proceeds while the guard runs** — tests, evidence, disclosures, documents. **A probe-path edit that becomes necessary after the guard has started waits for the next batch and its own run; it does not interrupt the one in flight.** | R266 | R266– |
| **What that costs, measured rather than asserted.** The guard's own timings file records three runs across R263–R265 at 484 s, 421 s and 495 s — **1,400 seconds, 23 minutes of wall time** — for three rounds that each edited a probe-path file and re-ran the guard once. **That is the floor and not the waste.** The waste is a second run inside one round because a line moved after the first had started, and the rounds above came close to it more than once. **The mechanism is sequencing, not patience.** | R266 | R266– |
