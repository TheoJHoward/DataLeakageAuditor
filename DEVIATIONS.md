...
## D-V30A-60 — the portability digest was quoted for fifteen rounds and cannot be regenerated by anyone

**True.** `15dc83c78950d42b…` was this project's portability evidence — the number
behind "the probe's answer did not depend on its environment" — and it appears in
`INSTALL.md` and `METHOD_VERIFICATIONS.md` across roughly fifteen rounds.
**Nobody can reproduce it, including its author.**

**Measured rather than suspected.** Sixteen plausible conventions were computed
over the five stored output bodies from the original runs: `\n` and `\r\n` joins,
each with and without a trailing separator, over the body alone, the body with the
banner, and every line; plus the raw file bytes, the stripped bytes, and the bytes
from the `VERDICT` offset. **None begins `15dc83c7`.** The five bodies hash
**identically to one another** under every one of those conventions.

**So the finding stands and the number does not, and those are different
statements.** The artifacts agree with each other, which is what the portability
claim rested on. What was lost is the recipe: what was hashed, in what order, with
what line endings, what trailing byte, what encoding. A digest whose recipe is
gone cannot be checked against anything, including a later run of the same code.

**And the inputs were unreachable too**, which is the part that made it
unreproducible by anyone else even holding the recipe: the run read its frames
from a session scratch directory that is not in the repository.

**A bare hash is the most authoritative-looking unframed figure there is.** That
is why this one survived fifteen quotations unchallenged while the same rounds
were enforcing "no figure without its frame" on the tool's own output.

**RETIRED, not corrected.** It is replaced by
`ddb133ff2fc959f0efb24124ca4d0f9dfc70f528cd5e1892136436c8dc34d9f1`, measured at
the declared floor corner — CPython 3.11.9 with numpy 1.26.4, pandas 2.1.4,
pyarrow 14.0.2 — and at the current resolution — CPython 3.12.10 with numpy 2.4.2,
pandas 3.0.1, pyarrow 23.0.1. **That spans a Python minor version and the whole
declared dependency range in one comparison, which the retired number never did.**

**A second reason is true and is deliberately not the headline.** The code has
moved by one line since: the stored 3 September bodies and today's differ by
exactly one note, the comparator line added when the tie branch was wired. Told
alone, that would be a softer story than the facts support — a digest whose recipe
is gone is unusable even at the commit it was taken from.

**The repair is an instrument, not a convention.** `tools/portability_digest.py`
prints the body it hashes, the rule it hashes it by, and the environment it ran
in; its inputs live in `tests/phase1/portability_data/`, carried in verbatim so
the number stays continuous with the stored bodies. Each clause of the recipe is
asserted in `tests/phase1/test_portability_digest.py`, because a prose description
of a convention decays exactly as the last one did. `OPERATING_RULES.md` §2
carries the rule.

**Expected:** that a digest offered as evidence can be recomputed by a reader,
which needs the recipe and the inputs, not the number.

## D-V30A-61 — the floor had no guard, and the tools had no enumeration

**Two gaps with one shape, disclosed together because the second was found while
closing the first.**

**The floor had no guard.** A change to a module on the probe's path re-runs the
whole-frame guard, which compares eight terms against a committed baseline. A
change that breaks the declared floor interpreter ran into nothing at all — which
is how `watch()` moved onto a 3.12-only recorder sixty-three minutes after the
floor was measured and stayed broken for two days (D-V30A-58). The floor row was a
hand-typed number in `INSTALL.md`: a measurement of one commit, written as a
standing fact about the package, with nothing connecting it to the tree.

`tools/floor_check.py` closes it. It takes the floor interpreter as a **required
argument** — no discovery, no default, no most-recently-modified heuristic, because
a floor check that picked its own environment would report confidently about
whichever one it found, which is D-V30A-48's defect — and it emits the
interpreter, the four package versions, the suite counts with their failures, the
digest with its recipe, and the `INSTALL.md` row ready to paste. **Cost, measured:
45 seconds wall clock**, which is a cadence anybody will actually wait for.

**The tools had no enumeration.** R228 said "other tools are unexamined, not
confirmed clean" — correct phrasing, and an absence claim with no population.
`tests/phase1/test_tools_floor_coverage.py` partitions `tools/` into COVERED — a
test imports it, named, so the floor suite run executed it — and UNEXAMINED, with
a reason. Disjoint, covering, and a new tool fails the file until it is placed.

**It found one on its first run, and then a second on the run after.**
`work_root_baseline` is UNEXAMINED: `test_work_root_baseline.py` tests the check
that *reads* the baseline and not the writer that produces it, so the writer's
behaviour on the floor interpreter is unmeasured. And `floor_check` itself failed
the enumeration the moment it was added, which is the totality guard doing its
job on the file that created it.
